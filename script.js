...
const S = {
  ...
  tab: 'daily', selDate: new Date(),
  wkOffset: 0, moOffset: 0, annYear: new Date().getFullYear(),
  modal: null, selCalDay: null,
  _mType: 'expense',
...
function vDaily() {
  ...
  const schedHTML = schedItems.length === 0
...
<!-- Mood + Weather side by side on large screens -->
<div class="card-pair">
  <div class="card">
  ...
  <div class="card">
    ...
    <div style="display:flex;gap:6px;">${WEATHERS.map(w => `<button onclick="setWeather(${w.id})" style="flex:1;background:${p.weather === w.id ? C.accent + '18' : 'none'};border:1.5px solid ${p.weather === w.id ? C.accent : C.border};border-radius:12px;padding:10px 4px;cursor:pointer;transition:all .2s;"><div style="font-size:24px;">${w.emoji}</div><div style="font-size:8px;font-family:var(--mono);color:${p.weather === w.id ? C.accent : C.muted};margin-top:4px;">${w.label}</div></button>`).join('')}</div>
  </div>
</div>

<!-- Schedule + To-Do side by side -->
<div class="card-pair">
  <div class="card">
...
<!-- Habits Today + Steps side by side -->
<div class="card-pair">
  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
...
<div class="card card-solo">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:${entries.length ? 12 : 0}px;">
    ${[['IN', `₹${inc.toFixed(2)}`, C.green], ['OUT', `₹${exp.toFixed(2)}`, C.red], ['NET', `₹${net.toFixed(2)}`, net >= 0 ? C.green : C.red]].map(([l, v, col]) => `<div style="text-align:center;background:${col}12;border:1px solid ${col}30;border-radius:10px;padding:10px 8px;"><div style="font-size:9px;font-family:var(--mono);color:${C.muted};">${l}</div><div style="font-size:14px;font-weight:700;color:${col};font-family:var(--mono);">${v}</div></div>`).join('')}
  </div>
  ${entries.map(e => `<div class="money-row"><div style="width:8px;height:8px;border-radius:99px;background:${e.type === 'income' ? C.green : C.red};flex-shrink:0;"></div><span style="flex:1;font-size:13px;">${esc(e.description)}</span>${e.category ? `<span style="font-size:10px;font-family:var(--mono);color:${C.muted};">${esc(e.category)}</span>` : ''}<span style="font-size:14px;font-weight:700;font-family:var(--mono);color:${e.type === 'income' ? C.green : C.red};">${e.type === 'income' ? '+' : '-'}₹${e.amount.toFixed(2)}</span><button class="icon-btn" onclick="deleteMoney('${e.id}')">✕</button></div>`).join('')}
</div>
...
<div class="wk-cols card-solo" style="margin-bottom:14px;">
  ${wdays.map(date => {
    const pp = getDayPlan(date), today = isToday(date), sv = pp.steps || 0, sok = sv >= STEPS_GOAL * .5;
...
<div class="card-pair">
  <div class="card">
    ...
    ${S.habits.length === 0 ? `<div style="font-size:12px;font-family:var(--mono);color:${C.muted};">No habits to show</div>` : `
    ...
    ${S.habits.map(h => { const cnt = wdays.filter(d => (getDayPlan(d).habitsDone || []).includes(h.id)).length; return `<div class="hm-row"><div style="flex:1;font-size:12px;display:flex;align-items:center;gap:5px;overflow:hidden;"><span>${h.emoji}</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(h.name)}</span></div>${wdays.map(d => { const done = (getDayPlan(d).habitsDone || []).includes(h.id); return `<div style="width:24px;height:24px;border-radius:5px;background:${done ? h.color : h.color + '20'};display:flex;align-items:center;justify-content:center;">${done ? `<span style="font-size:10px;font-weight:900;color:#000;">✓</span>` : ''}</div>`; }).join('')}<div style="width:36px;font-size:10px;font-family:var(--mono);color:${h.color};text-align:center;">${Math.round(cnt / 7 * 100)}%</div></div>`; }).join('')}`}
  </div>
  <div class="card">
...
<!-- Calendar + Day detail side by side -->
<div class="card-pair">
  <div class="card">
  ...
  ${sel ? `<div class="card" style="border-color:${C.accent}40;">${(() => { const sp = getDayPlan(new Date(sel)), si = (sp.moneyEntries || []).filter(e => e.type === 'income').reduce((a, e) => a + e.amount, 0), se = (sp.moneyEntries || []).filter(e => e.type === 'expense').reduce((a, e) => a + e.amount, 0); return `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span class="slabel" style="margin-bottom:0;">${fmt(sel, { weekday: 'long', month: 'long', day: 'numeric' })}</span><button class="today-pill" onclick="jumpToDate('${dk(new Date(sel))}')">View →</button></div>${sp.mood != null ? `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;"><span style="font-size:11px;font-family:var(--mono);color:${C.muted};">Mood:</span><span>${MOODS[sp.mood]?.emoji} ${MOODS[sp.mood]?.label}</span></div>` : ''} ${sp.steps ? `<div style="margin-bottom:6px;"><span style="font-size:11px;font-family:var(--mono);color:${C.muted};">Steps:</span> <span style="font-family:var(--mono);color:${C.blue};">👟${sp.steps.toLocaleString()}</span></div>` : ''} ${sp.habitsDone.length > 0 ? `<div style="margin-bottom:6px;"><span style="font-size:11px;font-family:var(--mono);color:${C.muted};">Habits:</span> ${S.habits.filter(h => sp.habitsDone.includes(h.id)).map(h => `<span>${h.emoji}</span>`).join('')}</div>` : ''} ${(sp.todos || []).filter(t => t.isDone).length > 0 ? `<div style="margin-bottom:6px;"><span style="font-size:11px;font-family:var(--mono);color:${C.muted};">Tasks done:</span> <span style="color:${C.blue};">${(sp.todos || []).filter(t => t.isDone).length}</span></div>` : ''} ${sp.notes ? `<div style="font-size:13px;color:${C.muted};margin-top:4px;">${esc(sp.notes.slice(0, 150))}</div>` : ''} ${(si > 0 || se > 0) ? `<div style="display:flex;gap:12px;margin-top:8px;">${si > 0 ? `<span style="font-size:12px;font-family:var(--mono);color:${C.green};">+₹${si.toFixed(0)}</span>` : ''}${se > 0 ? `<span style="font-size:12px;font-family:var(--mono);color:${C.red};">-₹${se.toFixed(0)}</span>` : ''}</div>` : ''}`; })()}</div>` : `<div class="card" style="display:flex;align-items:center;justify-content:center;min-height:200px;"><div style="text-align:center;color:${C.muted};font-size:13px;font-family:var(--mono);">Tap a day<br>to see details</div></div>`}
</div>
...
<!-- Activity + Mood heatmaps side by side -->
  <div class="card card-solo">
...
<div class="card-pair">
  <div class="card">
    ...
    ${ranked.map(({ h, done, pct }, i) => `<div style="margin-bottom:12px;"><div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;"><span style="font-size:12px;font-family:var(--mono);color:${C.muted};width:20px;">#${i + 1}</span><span>${h.emoji}</span><span style="flex:1;font-size:13px;">${esc(h.name)}</span><span style="font-size:11px;font-family:var(--mono);color:${h.color};">${done}d · ${Math.round(pct * 100)}%</span></div>${pbar(pct, h.color)}</div>`).join('')}
  </div>
  <div class="card">
    <span class="slabel">Mood Distribution</span>
    <div style="display:flex;gap:8px;align-items:flex-end;height:70px;">${MOODS.map(mood => { const cnt = allPlans.filter(p => p.mood === mood.id).length, h = moodDays.length > 0 ? Math.max(cnt / moodDays.length * 70, 2) : 2; return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;justify-content:flex-end;"><div style="font-size:9px;font-family:var(--mono);color:${C.muted};">${cnt}</div><div style="width:100%;height:${h}px;background:${mood.color};border-radius:3px 3px 0 0;"></div><div style="font-size:16px;">${mood.emoji}</div></div>`; }).join('')}</div>
  </div>
</div>
...
function vInsights() {
  ...
  return `<div class="card card-solo"><span class="slabel">30-Day Overview</span><div style="display:flex;justify-content:space-around;flex-wrap:wrap;gap:12px;">${[['Habits', habRate, C.accent], ['Tasks', taskRate, C.blue], ['Logged', moodRate, C.yellow], ['Steps', stepsGoalDays / 30, C.green]].map(([l, p, col]) => `<div style="display:flex;flex-direction:column;align-items:center;gap:6px;">${ring(p, col, 68, `<span style="font-size:13px;font-weight:700;color:${C.text};">${Math.round(p * 100)}%</span>`)}<span style="font-size:9px;font-family:var(--mono);color:${C.muted};">${l}</span></div>`).join('')}</div></div>
...
<!-- Mood chart + Habit leaderboard side by side -->
<div class="card-pair">
  <div class="card">
...
function vProfile() {
  ...
  return `
<!-- Profile header + Stats side by side -->
<div class="card-pair">
  <div class="card">
    <div style="display:flex;align-items:center;gap:14px;">
      <div class="profile-ava">${initials}</div>
      <div>
        <div style="font-size:18px;font-weight:600;">${esc(p.fullName)}</div>
  ...
  <div class="card">
    ...
    ${[['📅', 'Days logged', Object.keys(S.dayPlans).length], ['🔥', 'Habits tracked', S.habits.length], ['👟', 'Steps this year', yearSteps.toLocaleString()], bestH ? ['🏆', 'Best streak', `${bestH.longestStreak || 0}d · ${bestH.name}`] : null].filter(Boolean).map(([icon, label, value]) => `<div class="stat-row"><span style="font-size:14px;">${icon} ${label}</span><span style="font-size:13px;color:${C.muted};font-family:var(--mono);">${value}</span></div>`).join('')}
  </div>
</div>
...
<div class="card card-solo">
  ...
  <button onclick="confirmClear()" style="background:${C.red}20;border:1px solid ${C.red}40;color:${C.red};border-radius:10px;padding:10px 16px;cursor:pointer;font-size:14px;width:100%;text-align:left;">🗑 Clear All Data</button>
</div>
...
function renderModal() {
  ...
  div.id = 'modalOverlay'; div.className = 'modal-bg';
...
function toggleHabitDay(habitId) {
  ...
  saveDayPlan(p);
  S.habits = S.habits.map(h => { if (h.id !== habitId) return h; const ns = was ? Math.max(0, (h.currentStreak || 1) - 1) : (h.currentStreak || 0) + 1; return { ...h, currentStreak: ns, longestStreak: Math.max(h.longestStreak || 0, ns) }; });
  saveHabits(); renderContent()
}
...
function renderOnboard() {
  ...
  if (!el) { el = document.createElement('div'); el.id = 'onboard'; document.getElementById('app').appendChild(el); }
...
window.addEventListener('resize', () => { setVH(); render() });
setVH();
render();